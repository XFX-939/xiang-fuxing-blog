      ...
      <div className="mt-5 flex flex-wrap gap-3">
        {project.links.map((link) => (
          <Link
            key={link.href}
            href={link.href}
            className="inline-flex min-h-11 w-full items-center justify-center gap-1 rounded-md border border-border bg-surface-elevated px-4 py-2.5 text-sm font-semibold text-accent transition hover:border-accent hover:bg-accent-soft sm:min-h-0 sm:w-auto sm:border-0 sm:bg-transparent sm:px-0 sm:py-0 sm:font-medium"
          >
            {link.label}
